...
CREATE VIEW vw_detalhes_pedidos AS
    ...
    FROM
        tb_pedido_roupa pr
            JOIN
        tb_cliente cl ON pr.CLIENTE_id_cliente = cl.id_cliente
            JOIN
        tb_roupa r ON pr.ROUPA_id_roupa = r.id_roupa
            JOIN
        tb_cor c ON r.COR_id_cor = c.id_cor;
...
LEFT JOIN tb_roupa r ON c2.id_categoria = r.CATEGORIA_id_categoria
WHERE c1.CATEGORIA_id_categoria_pai IS NULL;
